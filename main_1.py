...
def test_1():
    ...
    for flat_iterator_item, check_item in zip(
            FlatIteratorAllLevel(list_of_lists_1),
            ['a', 'b', 'c', 'd', 'e', 'f', 'h', False, 1, 2, None]
    ):

        assert flat_iterator_item == check_item

    assert list(FlatIteratorAllLevel(list_of_lists_1)) == ['a', 'b', 'c', 'd', 'e', 'f', 'h', False, 1, 2, None]
    print('Функция test_1() отработала без ошибок!')
